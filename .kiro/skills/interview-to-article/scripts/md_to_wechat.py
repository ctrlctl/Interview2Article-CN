...
def inline_images(html, base_dir):
    ...
    are left alone with a warning on stderr. Images are resized to fit WeChat's
    677px column width.
    """
    ...
    def repl(m):
        ...
        print(f"  inlined {src} ({buf.tell() // 1024} KB, {img.width}x{img.height})", file=sys.stderr)
...
def style_figure_captions(html):
    """Style image captions (e.g. '*▲ Neo Trinity*') — centered, muted, tight to image.

    Handles two cases:
    1. Caption in same <p> as image, separated by <br>
    2. Caption as standalone <p><em>▲...</em></p>
    """
    # Case 1: <img ...><br/>\n<em>▲ ...</em></p>  →  split into image + styled caption
    html = re.sub(
        r'(<img [^>]+>)\s*<br\s*/?>\s*\n?\s*(<em[^>]*>▲[^<]*</em>)\s*</p>',
        lambda m: (
            m.group(1) + '</p>\n'
...
def style_heading_underlines(html):
    """Wrap h2 text in a span with border-bottom so underline only covers text width."""
    def repl(m):
        tag_open = m.group(1)
        content = m.group(2)
        # Inject inline-block span with underline inside the centered h2
        return (
            f'{tag_open}'
            f'<span style="display:inline-block;padding-bottom:8px;'
            f'border-bottom:1px solid #1a1a1a;">{content}</span></h2>'
        )
