...
2. **If a URL is provided:**
   - Run `python extract_srt.py <URL>` to download and transcribe. Language defaults to `auto` — the script samples 30s from the middle of the audio to detect the dominant language (important for videos where intro language differs from main content, e.g. a Chinese host opening an English interview).
   - Override only if detection fails: `python extract_srt.py <URL> zh` (or `en`, `ja`, etc.).
   - This uses yt-dlp to download the audio and Whisper to generate an SRT file.
   - If yt-dlp does not support the URL (e.g., WeChat 视频号), inform the user and ask them to provide the video file or transcript manually.
3. **If a local video/audio file is provided:**
   - Run `python extract_srt.py <file_path>` (the script auto-detects local files and skips download). Same `auto` language behavior applies.
4. **If a transcript/subtitle file (SRT, VTT, TXT) is provided:**
   - Use it directly.
5. Parse the SRT/transcript and output `raw_transcript.md` in the workspace.
...
Based on the topics identified, carefully review the transcript for speech recognition errors:
...
1. **Brand & Product Names (品牌与产品识别):**
   ...
   - Save a list to `brands_products.md` with format:
     ```
     | Brand/品牌 | Products/产品 | First Mention Location |
     ```

2. **Context-based Correction (基于主题的纠错):**
   - Use the topic context to fix obvious transcription errors.
...
3. **Uncertain Segments (需人工确认的片段):**
   ...
   - Collect all uncertain segments into `review_needed.md`.
